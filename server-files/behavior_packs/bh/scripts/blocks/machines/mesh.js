...
import { sievableBlocks, meshMulti, } from '../machines/a_machines_config.js'
    ...
    eventData.blockComponentRegistry.registerCustomComponent('twm:mesh', {
        onPlayerInteract(e) {
            ...
            if (mainHand) {
                ...
                if (blockType == 'empty') {
                    const sievableBlock = sievableBlocks[mainHand.typeId]
...
function finishFiltering(block) {
    ...
    for (const sieve of Object.values(sieves)) {
        ...
        const multi = meshMulti['twm:' + meshState]
        if (!multi) continue

        const sievableBlock = sievableBlocks[sieve.permutation.getState('twm:block')]
