...
import { pressableItems, electroPressSettings, speedCalculate, energyEfficienyCalculate } from './a_machines_config.js'

const { energyCost, energyCap, rateSpeedBase } = electroPressSettings

world.beforeEvents.worldInitialize.subscribe(eventData => {
    eventData.blockComponentRegistry.registerCustomComponent('twm:electro_press', {
        beforeOnPlayerPlace(e) {
            ...
            system.run(() => {
                ...
                entity.nameTag = 'entity.twm:electro_press.name'
        ...
        onTick(e) {
            ...
            const itemToProcess = pressableItems[item?.typeId]
            ...
            if (itemToProcess.required > item.amount) {
                displayBar(energy, energyCap, rateSpeedBase * speedMulti, inv, 'energy', energyEfficiency);
                returnFunction(entity, block, inv, 4)
                return;
            }
            ...
            if (progress >= energyCost) {
                let processCount = Math.min(Math.floor(progress / energyCost), Math.floor(item.amount / itemToProcess.required));
                inv.addItem(new ItemStack(itemToProcess.output, processCount));
                progress -= processCount * energyCost;
                item.amount > processCount * itemToProcess.required ? item.amount -= processCount * itemToProcess.required : item = undefined;
                inv.setItem(3, item);
            } else {
